...
void buildTag(E_Int* cNG, FldArrayI& posFaces,
              E_Float* f1x, E_Float* f1y, E_Float* f1z,
              E_Float* f2x, E_Float* f2y, E_Float* f2z,
              E_Int* tagp, 
              E_Int& collapsed, E_Int& expanded)
{
  E_Int nfaces = posFaces.getSize();
  ...
  for (E_Int i = 0; i < nfaces; i++)
  {
   ...
   // Critere d'aretes inversees
   s = SCAL(v1x,v1y,v1z,v2x,v2y,v2z);
   if (s < 0)  { tagp[ind1] = -1; tagp[ind2] = -1; collapsed++; } // collapse
 }
}

// Calcul du tag au noeud
void buildTag2(E_Int* cNG, E_Int npts, 
               FldArrayI& posFaces, vector< vector<E_Int> >& cVF,
               E_Float* f1x, E_Float* f1y, E_Float* f1z,
               E_Float* f2x, E_Float* f2y, E_Float* f2z,
               E_Int* tagp, 
               E_Int& collapsed, E_Int& expanded)
{
  ...
  E_Float x1, x2, y1, y2, z1, z2, l1, l2, d, xp1, xp2, yp1, yp2, zp1, zp2;
  E_Float v1x,v1y,v1z,v2x,v2y,v2z,d1x,d1y,d1z,r1x,r1y,r1z,r2x,r2y,r2z,s;
  E_Float v3x, v3y, v3z, v4x, v4y, v4z, x3, y3, z3, xp3, yp3, zp3;
  E_Float v5x, v5y, v5z, d1, d2, angle, norm;
  E_Int indv1, indv2;
  ...
  {
    x1 = f1x[i]; y1 = f1y[i]; z1 = f1z[i];
    xp1 = x1 + f2x[i]; yp1 = y1 + f2y[i]; zp1 = z1 + f2z[i];
    vector<E_Int>& f = cVF[i]; // faces incidente a i
    E_Int nbf = f.size(); 
    for (E_Int f1 = 0; f1 < nbf; f1++)
      for (E_Int f2 = f1+1; f2 < nbf; f2++)
      {
        pt = cNG+posFaces[f[f1]-1];
        ind1 = pt[1]-1; ind2 = pt[2]-1;  
        if (ind1 == i) {indv1 = ind2; x2 = f1x[ind2]; y2 = f1y[ind2]; z2 = f1z[ind2]; 
        xp2 = x2 + f2x[ind2]; yp2 = y2 + f2y[ind2]; zp2 = z2 + f2z[ind2];}
        else if (ind2 == i) {indv1 = ind1; x2 = f1x[ind1]; y2 = f1y[ind1]; z2 = f1z[ind1]; 
        xp2 = x2 + f2x[ind1]; yp2 = y2 + f2y[ind1]; zp2 = z2 + f2z[ind1];}
        pt = cNG+posFaces[f[f2]-1];
        ind1 = pt[1]-1; ind2 = pt[2]-1;  
        if (ind1 == i) {indv2 = ind2; x3 = f1x[ind2]; y3 = f1y[ind2]; z3 = f1z[ind2]; 
        xp3 = x3 + f2x[ind2]; yp3 = y3 + f2y[ind2]; zp3 = z3 + f2z[ind2];}
        else if (ind2 == i) {indv2 = ind1; x3 = f1x[ind1]; y3 = f1y[ind1]; z3 = f1z[ind1]; 
        xp3 = x3 + f2x[ind1]; yp3 = y3 + f2y[ind1]; zp3 = z3 + f2z[ind1];}
        v1x = x2-x1; v1y = y2-y1; v1z = z2-z1;
        v2x = xp2-xp1; v2y = yp2-yp1; v2z = zp2-zp1;
        v3x = x3-x1; v3y = y3-y1; v3z = z3-z1;
        v4x = xp3-xp1; v4y = yp3-yp1; v4z = zp3-zp1;
        
        r1x = VECTX(v1x,v1y,v1z,v3x,v3y,v3z);
        r1y = VECTY(v1x,v1y,v1z,v3x,v3y,v3z);
        r1z = VECTZ(v1x,v1y,v1z,v3x,v3y,v3z);

        r2x = VECTX(v2x,v2y,v2z,v4x,v4y,v4z);
        r2y = VECTY(v2x,v2y,v2z,v4x,v4y,v4z);
        r2z = VECTZ(v2x,v2y,v2z,v4x,v4y,v4z);
        s = SCAL(r1x,r1y,r1z,r2x,r2y,r2z);
        if (s < -1.e-6) { tagp[i] = -1; tagp[indv1] = -1; tagp[indv2] = -1;}

        v5x = xp3-xp2; v5y = yp3-yp2; v5z = zp3-zp2;
        d1 = NORM2(v2x,v2y,v2z); d1 = sqrt(d1);
        d2 = NORM2(v4x,v4y,v4z); d2 = sqrt(d2);
        v2x = v2x/d1; v2y = v2y/d1; v2z = v2z/d1;
        v4x = v4x/d2; v4y = v4y/d2; v4z = v4z/d2;
        xp3 = xp1+v4x; yp3 = yp1+v4y; zp3 = zp1+v4z;
        xp2 = xp1+v2x; yp2 = yp1+v2y; zp2 = zp1+v2z;
        v5x = xp3-xp2; v5y = yp3-yp2; v5z = zp3-zp2;
        d2 = NORM2(v5x,v5y,v5z);
        if (d2 < 0.1)  { tagp[i] = -1; tagp[indv1] = -1; tagp[indv2] = -1; }
        ...
        norm = NORM2(r2x,r2y,r2z);
        norm = sqrt(norm);
        if (norm > 1.e-10)
        {
          dirVect[0] = r2x/norm; dirVect[1] = r2y/norm; dirVect[2] = r2z/norm;
        }
        else
        {
        ...
        //dirVect[0] = 1; dirVect[1] = 0; dirVect[2] = 0;
        angle = K_COMPGEOM::getAlphaAngleBetweenBars(p1, p2, 
                                                     p1, p3,
                                                     dirVect);
        if (angle < 20.)
        {
          // tagp[i] = -1; tagp[indv1] = -1; tagp[indv2] = -1;
          tagp[i] = (int)angle;
        }
        */
      }
      if (tagp[i] == -1) collapsed++;
  }
}

// Calcul du tag au noeud
void doCollapse(E_Int* cNG, E_Int npts, 
                FldArrayI& posFaces, vector< vector<E_Int> >& cVF,
                E_Float* f1x, E_Float* f1y, E_Float* f1z,
                E_Float* f2x, E_Float* f2y, E_Float* f2z,
                E_Int* tagp, 
                E_Int& collapsed, E_Int& expanded)
{
  ...
  E_Float v3x, v3y, v3z, v4x, v4y, v4z, x3, y3, z3, xp3, yp3, zp3;
  E_Int indv1, indv2;
  E_Float v5x, v5y, v5z, xf, yf, zf, d1, d2, angle;
  E_Float p1[3]; E_Float p2[3]; E_Float p3[3];
  E_Float dirVect[3];
  for (E_Int i = 0; i < npts; i++)
  {
    ...
    for (E_Int f1 = 0; f1 < nbf; f1++)
      for (E_Int f2 = f1+1; f2 < nbf; f2++)
      {
        ...
        if (s < -1.e-6) 
        { 
          xf = 0.5*(xp2+xp3); yf = 0.5*(yp2+yp3); zf = 0.5*(zp2+zp3);
          f2x[i] = xf-x1; f2y[i] = yf-y1; f2z[i] = zf-z1;
          f2x[indv1] = xf-x2; f2y[indv1] = yf-y2; f2z[indv1] = zf-z2;
          f2x[indv2] = xf-x3; f2y[indv2] = yf-y3; f2z[indv2] = zf-z3;
        }
        ...
        //xp2 = xp1+v2x; yp2 = yp1+v2y; zp2 = zp1+v2z;
        //v5x = xp3-xp2; v5y = yp3-yp2; v5z = zp3-zp2;
        v5x = v4x-v2x; v5y = v4y-v2y; v5z = v4z-v2z; 
        d2 = NORM2(v5x,v5y,v5z);
        if (d2 < 0.01)  
        {
          xf = 0.5*(xp2+xp3); yf = 0.5*(yp2+yp3); zf = 0.5*(zp2+zp3);
        ...
        p2[0] = xp2; p2[1] = yp2; p2[2] = zp2;
        p3[0] = xp3; p3[1] = yp3; p3[2] = zp3;
        dirVect[0] = r2x; dirVect[1] = r2y; dirVect[2] = r2z;
        angle = K_COMPGEOM::getAlphaAngleBetweenBars(p1, p2, 
                                                     p1, p3,
                                                     dirVect);
        if (angle < 20.)
...
PyObject* K_TRANSFORM::deform2(PyObject* self, PyObject* args)
{
  ...
  E_Float reducFactor = 0.9;
  E_Int collapsed = 1, expanded = 0;
  E_Int count = 0;

  /*
  while (collapsed > 0 && count < 3)
  {
    ...
    // Boucle de reduction de f2
    for (E_Int i = 0; i < npts; i++)
    {
      if (tagp[i] == -1) 
      { 
        f2x[i] = reducFactor*f2x[i];
        f2y[i] = reducFactor*f2y[i];
        f2z[i] = reducFactor*f2z[i];
        printf("reducing %d\n", i);
      }
    }
    count += 1;
  }
  printf("Final : Expanded=%d, collapsed=%d\n", expanded, collapsed);
  */
  ...
  */
  ...
  // Ajoute tag a la sortie
  E_Float* tagpp = newFp+3*nptsNew;
  for (E_Int i = 0; i < npts; i++) tagpp[i] = tagp[i];
