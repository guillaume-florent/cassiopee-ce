...
# - octree -
# octree: evidement de faible hauteur
...
# INPUT: Closed geometry, properly meshed avec un pas a peu pres
...
dfar = -1  # distance des frontieres lointaine du corps
hWall = 0.001  # 0. #0.001 # hauteur de la premiere maille(si prismes, 0. sinon)
...
if snear < 0:
    ...
    print('surface area min/max/mean=%f,%f,%f' % (minarea, maxarea, meanarea))
    snear = 2.*meanarea**(1./2.)
    print('snear=%f' % snear)
...
if hWall > 0.:
    ...
    secondlayer = True
    smoothIter = 100
    if algoBL == 0:  # distribution selon raison geometrique 1.2
        raison = 1.2
        nlayer = int(1+math.log(snear/(8.*hWall))/math.log(raison))
        print('nb de couches de prismes = %i' % nlayer)
        dlayer = hWall*(1.-raison**nlayer)/(1.-raison)
        print('epaisseur de la couche de prismes = %f' % dlayer)
        d = G.cart((0., 0., 0.), (dlayer/nlayer, 1, 1), (nlayer, 1, 1))
        for i in xrange(0, nlayer):
            C.setValue(d, 'CoordinateX', i, hWall*(1.-raison**i)/(1.-raison))
        hLast = C.getValue(d, 'CoordinateX', nlayer-1)-C.getValue(d,
                                                                  'CoordinateX',
                                                                  nlayer-2)
        print('hauteur de la derniere couche de prisme = %f' % hLast)
        lay = G.addNormalLayers(s, d, check=0, niter=smoothIter)
        if secondlayer:
            # 2eme couche avec raison geometrique 2.
            ...
            hWall = hLast
            raison = 2.
            nlayer = int(1+math.log(snear/(1.*hWall))/math.log(raison))
            print('nb de couches de prismes = %i' % nlayer)
            dlayer = hWall*(1-raison**nlayer)/(1-raison)
            print('epaisseur de la couche de prismes = %f' % dlayer)
            d = G.cart((0., 0., 0.), (dlayer/nlayer, 1, 1), (nlayer, 1, 1))
            ...
            lay = T.join(lay, lay1)
    else:  # distribution avec G.enforce
        nlayer = 41
        Nr = int(0.3*nlayer)
        Nr2 = int(1.*nlayer)
    ...
    if snear < 0.:  # snear auto
        ...
        minarea = C.getMinValue(mesh, 'centers:sMagnitude')
        maxarea = C.getMaxValue(mesh, 'centers:sMagnitude')
        print('surface area min/max/mean=%f, %f, %f' % (minarea,
                                                        maxarea,
                                                        meanarea))
        snear = 2.*meanarea**(1./2.)
        print('snear=%f' % snear)
...
for e in ext:
    bb = G.bbox(e)
    if bb[0] < bbox[0] or bb[3] > bbox[3] or bb[1] < bbox[1] or bb[4] > bbox[4] or bb[2] < bbox[2] or bb[5] > bbox[5]:
        pass
    else:
        ext = e
        break
...
for zone in Internal.getNodesFromType2(ext, 'Zone_t'):
    if C.getMaxValue(zone, 'CoordinateX') != XMax_surf:
        bc[2][1][2].append(zone)  # suppression de la surface du corps
