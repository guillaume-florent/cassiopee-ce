...
/*
  ...
  {
    E_Float mi = _metric[Ni];
    E_Float mj = _metric[Nj];
    E_Float d = ::sqrt(K_FUNC::sqrDistance(_pos.col(Ni), _pos.col(Nj), _pos.rows()));
    _metric[Ni] = std::min (mi, mj + eps*d);
    _metric[Nj] = std::min (mj, mi + eps*d);
  }
