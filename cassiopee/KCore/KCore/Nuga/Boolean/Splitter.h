...
namespace NUGA
{
  ...
  public: //PG SPLITTING
    ...
    inline static E_Int triangulate_external_pgs(const K_FLD::FloatArray& crd, ngon_type& ngi, ngon_type& ngo);
    ///
    template <typename TriangulatorType>
  ...
  public: // PH SPLITTING
    ...
    (K_FLD::FloatArray& crd, ngon_type& ngi, E_Float concave_threshold, E_Float convex_threshold, E_Float RTOL, ngon_type& ngo)
    {
      ...
      for (size_t i=0; i < PHtypes.size(); ++i) if (PHtypes[i] != ngon_type::PATHO_PH_NONE) PHlist.push_back(i);
    ...
    (K_FLD::FloatArray& crd, const ngon_type& ngi, const ngon_unit& orient, E_Float concave_threshold, E_Float convex_threshold, E_Float RTOL, ngon_type& ngo, const Vector_t<E_Int>& PHlist)
    {
      ...
      E_Int nb_err(0),nb_fixed(0), nb_false(0);
      ...
      std::cout << "nb of successfull splits : " << nb_fixed << " over " << nb_phs << " elements." << std::endl;
      std::cout << "nb of non-necessary splits : " << nb_false << " over " << nb_phs << " elements." << std::endl;
      std::cout << "nb of non-handled (unsplittable) elements : " << nb_err << " over " << nb_phs << " elements." << std::endl;
  ...
  private:
    ...
    inline static bool __valid_split(const K_FLD::FloatArray& crd, const std::vector<E_Int>& chain, E_Int index_start,
                              ...
                              const ivec_t& simple_colors, const eset_t& reflex_edges, E_Float angle_max, E_Float chain_angle, E_Float & angle, ngon_type& twoPHs);
    ...
    (const ngon_unit& PGs, const K_FLD::FloatArray& crd, ngon_unit& convex_pgs, Vector_t<E_Int>& oids, const transfo_t& params, const Vector_t<bool>* process)
  {
    ...
    for (E_Int i = 0; (i<sz); ++i)
    {
      ...
      connectT3.clear();
      err = K_MESH::Polygon::triangulate(dt, crd, nodes, nb_nodes, 1/*index start*/, connectT3);
...
template <typename TriangulatorType>
E_Int NUGA::Splitter::triangulate_external_pgs(const K_FLD::FloatArray& crd, ngon_type& ngi, ngon_type& ngo)
{
...
E_Float concave_threshold, E_Float convex_threshold, E_Float rtol, ngon_type& twoPHs, const E_Float** normals)
{
  ...
  if (PHi == 160)
  {
    ivec_t oids, colors;
    K_FLD::IntArray cT3;
    E_Int err = ngon_type::triangulate_pgs<TriangulatorType>(pgs, crd, cT3, oids);
    colors.resize(cT3.cols(), 0);
    for (E_Int i = 0; i < cT3.cols(); ++i)
      colors[i] = simple_colors[oids[i]];
    MIO::write("simplified.mesh", crd, cT3, "TRI", 0, &colors);
  ...
  for (size_t c = 0; c < nb_chains; ++c)
  {
    ...
    if (!__valid_split<TriangulatorType>(crd, chain, index_start,
                       PGS, first_pg, nb_pgs, v, VOL_THRESHOLD, simple_colors, rflx_edges, angle_max, chain_angle[c], angle, twoPHs))
      continue;
...
const ngon_unit& PGS, const E_Int* first_pg, E_Int nb_pgs, E_Float V, E_Float VOL_THRESHOLD,
const ivec_t& simple_colors, const eset_t& reflex_edges, E_Float angle_max, E_Float chain_angle, E_Float & angle, ngon_type& twoPH)
{
  bool ok = true;
  angle = 0;
  twoPH.clear();
...
if (PHi == faultyPH)
{
  ...
  K_MESH::Polyhedron<UNKNOWN>::metrics(dt, crd, twoPH.PGs, pgs0, nb_pgs0, v1, G);
  K_MESH::Polyhedron<UNKNOWN>::metrics(dt, crd, twoPH.PGs, pgs1, nb_pgs1, v2, G);

  if (v1 < V * VOL_THRESHOLD) //null or two small (doesn't worth a cut)
    return false;
  if (v2 < V* VOL_THRESHOLD) //null or too small (doewnt worth a cut)
    return false;
