...
public:
  ...
  explicit AnisoMetricType(const E_Float* vals){ for (E_Int k = 0; k < DIMANISO; ++k)_mij[k] = vals[k];}
  explicit AnisoMetricType(const E_Float& val) { for (E_Int k = 0; k < DIMANISO; ++k)_mij[k] = val;}
